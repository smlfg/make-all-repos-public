...
Lineage: this repository was extracted from the Hermes-coupled prior art at `~/.config/hai-agent-mcp`, which it replaces for control-plane work. See [Legacy](#legacy) for the drift note.

## Start here

New to the repo? Read these first, in order:

1. `GOAL.md` — the current slice goal, in-scope/out-of-scope, and acceptance criteria.
2. `docs/TOOL_CONTRACT.md` — tool mutation/gate matrix, paths, artifact names, and error codes.
3. `docs/OWNER_GATE.md` — how the owner gate works and which modes exist.
4. `src/hai_mcp/server.py` — MCP tool registration and the transport entry point.
5. `AGENTS.md` — contributor rules for agents working in this repo.

For install and client wiring, jump to [Quick start](#quick-start).

## Repository layout

| Path | What lives here |
| --- | --- |
| `src/hai_mcp/` | Server, control-plane logic, mission engine, owner gate, confinement |
| `tests/` | Unit and gate tests |
| `docs/` | Contracts, owner-gate design, plans, wireframes, visuals, client snippets |
| `scripts/` | Standalone smoke and impact-eval entry points |
...
| `AGENTS.md` | Contributor rules for agents |
| `FOR_SMLFLG.md` | Owner-facing project notes |
...
These boundaries are load-bearing and are preserved from the project's original contract:

- **No LLM calls inside the server.** The server is deterministic; clients bring the model.
- **Owner gates fail closed.** A gated action without a valid owner code does not proceed.
- **No Git mutation.** The server never shells out to `git`, parses `.git`, commits, stashes, pushes, or touches repository state. Clients pass Git telemetry in; HAI-MCP only classifies it.
- **Confined state.** Writes stay inside `HAI_HOME`, `HAI_OWNER_HOME`, and the project's run-contract directory, with symlink-safe path handling.
- **Do not sync `HAI_HOME`.** Not with Dropbox, Syncthing, Git, or similar mirroring — the store holds local paths and lock-protected state.
- **HTTP is not an adversarial auth layer.** It is a way for multiple clients to reach one writer process; non-loopback binds require `HAI_HTTP_TOKEN`.
