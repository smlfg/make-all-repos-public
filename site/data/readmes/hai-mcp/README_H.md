...
2. `GOAL.md` — current slice goal and active boundaries
3. `docs/TOOL_CONTRACT.md` — full tool gate matrix, paths, artifact names, error codes
4. `docs/OWNER_GATE.md` — owner-gate design and modes
5. `docs/client-snippets/` — ready-made MCP client config examples
...
| Path | Purpose |
| --- | --- |
| `src/hai_mcp/` | Server source: transport, control plane, mission engine, owner gate, paths, projects |
| `docs/` | Contract docs, owner gate design, client snippets, build handoff, wireframes |
| `tests/` | Unit and gate tests with fixtures |
| `scripts/` | Evaluation and smoke-test scripts |
| `GOAL.md` | Active slice goal and boundaries |
| `AGENTS.md` | Contributor rules for agents working in this repo |

## Workflow and safety boundaries

HAI-MCP is a **fail-closed contract kernel**. The owner and the agent are separate principals. Every owner-gated action requires explicit approval; the server never auto-approves on behalf of the owner.

Hard boundaries the server enforces:

- **No LLM calls inside the server.** Models remain client-side.
- **No Git operations.** The server never shells out to `git`, parses `.git`, commits, stashes, or pushes. Git hygiene is telemetry-only.
- **No cloud sync of `HAI_HOME`.** The store contains local paths and `fcntl` locks. Do not sync with Dropbox, Syncthing, Git, or similar mirroring. Use the HTTP transport or a single writer process for multi-client access.
- **`HAI_OWNER_HOME` must not be inside `HAI_HOME`.** Owner approval codes live separately from control-plane state.
- **No commit, push, deploy, or delete tooling.** HAI-MCP manages contracts, not repository operations.
- **No semantic verification.** The server does not judge whether the model "really understood" the goal.
- **One writer process.** `fcntl` locks protect mutation authority; the HTTP transport is an additional pipe into that process, not a second writer.

Clients may report telemetry for `hai_check_git_hygiene` and `hai_check_source_freshness`, but the server never opens external sources or mutates repository state based on that telemetry.

## Lineage and drift

This repo was extracted from the parent HAI workspace (`<local path>`). The legacy Hermes-coupled prior art at `~/.config/hai-agent-mcp` still exists; coexistence is fine until you switch clients deliberately. Active development follows the `GOAL.md` slice contract: one WIP slice at a time, owner gates fail-closed, no LLM calls inside the server.
