...
1. `GOAL.md` — current slice goal and active boundaries.
2. `AGENTS.md` — contributor rules for agents working in this repo.
3. `docs/TOOL_CONTRACT.md` — tool mutation/gate matrix, paths, artifact names, errors.
4. `docs/OWNER_GATE.md` — owner-gate design and modes.
5. `FOR_SMLFLG.md` — owner-facing notes for the current slice.
6. `docs/BUILD_HANDOFF.md` — build handoff and verification history (read as history, see lineage note below).

## Repository structure

| Path | Purpose |
| --- | --- |
| `README.md` | Identity, quickstart, structure, and safety boundaries (this file) |
| `GOAL.md` | Current slice goal and active boundaries |
| `AGENTS.md` | Contributor rules for agents working in this repo |
| `FOR_SMLFLG.md` | Owner-facing notes for the current slice |
| `pyproject.toml` | Package metadata, `hai-mcp` console entry, pytest config |
| `uv.lock` | Locked dependency set for `uv sync --all-extras` |
| `src/hai_mcp/server.py` | Server package entry: MCP tool registration and transport |
| `tests/conftest.py` | Test suite root; isolates HAI_HOME per session |
| `scripts/stdio_smoke.py` | Stdio smoke run of a real client against an isolated HAI_HOME |
...
| `docs/eval/AB_HARNESS_CONTRACT.md` | A/B harness contract for the `hai_distill` gate |
| `docs/client-snippets/README.md` | Index of MCP client config examples |
| `docs/visuals/hai-mcp-mission-lifecycle.svg` | Mission lifecycle diagram; PNG fallback in `docs/visuals/hai-mcp-mission-lifecycle.png` |
...
![HAI-MCP mission lifecycle](docs/visuals/hai-mcp-mission-lifecycle.svg)
...
These hold for every client that talks to this server:

- One WIP slice at a time; the active scope lives in `GOAL.md`.
- Owner gates fail closed — no valid one-time code means no gated mutation (`docs/OWNER_GATE.md`).
- The server never runs Git: no commit, no push, no stash, no status subprocess; clients pass hygiene telemetry instead.
- The server makes no LLM calls; it validates, writes confined local state, and returns structured JSON.
- It is not a commit, push, deploy, or delete tool (see "What it is not").
- Do not sync the HAI_HOME store with Dropbox, Syncthing, Git, or similar file mirroring (see "State and paths").
- Non-loopback HTTP binds require HAI_HTTP_TOKEN; the HTTP transport is not an owner-authentication layer (see "Transport").
- Changes stay inside this repository and its own state roots; live ~/.hermes, live ~/.hai, and unrelated HAI apps are out of scope.
...
Lineage/drift note: `docs/BUILD_HANDOFF.md` was written against the original build workspace and still names that path plus requirement sources (7coreFunctions, 7Functions) that are not part of this repository. Read it as history, not as a current instruction.
